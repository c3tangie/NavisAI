...
Use ARROWS or WASD keys for control.
    ...
    Add --navisvision-classifier to compare crop classifications per box.
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    pygame.display.set_mode(world.hud.dim, flags)
                    state = "Fullscreen" if self._fullscreen else "Windowed"
                    world.hud.notification("Display mode: %s" % state)
...
def draw_navisvision_detections(display, detections, font):
    """Draw detector boxes plus optional independent crop classifications."""
    ...
    for detection in detections:
        ...
        detector_caption = "DET: %s %.0f%%" % (
            detection["label"],
            detection["score"] * 100.0,
        )
        caption_rows = [(detector_caption, color)]
        if "classifier_label" in detection:
            classifier_color = (
                (65, 180, 95)
                if detection.get("classifier_agrees")
                else (185, 85, 210)
            )
            caption_rows.append(
                (
                    "CLS: %s %.0f%%" % (
                        detection["classifier_label"],
                        detection["classifier_score"] * 100.0,
                    ),
                    classifier_color,
                )
            )
        ...
        rendered_rows = [
            (font.render(caption, True, (255, 255, 255)), row_color)
            for caption, row_color in caption_rows
        ]
        row_height = max(
            surface.get_height() + 4 for surface, _ in rendered_rows
        )
        total_height = row_height * len(rendered_rows)
        label_y = y - total_height - 4
        if label_y < 0:
            label_y = min(display_height - total_height, y + 4)

        for row_index, (label_surface, row_color) in enumerate(
                rendered_rows):
            current_y = label_y + row_index * row_height
            ...
            pygame.draw.rect(display, row_color, background)
            display.blit(label_surface, (x + 3, current_y + 2))


class NavisVisionInferenceWorker:
    """Run detection off the CARLA/render thread and discard stale frames."""

    def __init__(self, runtime, classifier_runtime=None, maximum_fps=5.0):
        self.runtime = runtime
        self.classifier_runtime = classifier_runtime
        self.minimum_interval = 1.0 / float(maximum_fps)
        ...
        self._thread = threading.Thread(
            target=self._run,
            name='NavisVisionInference',
            daemon=True,
        )
        self._thread.start()

    def submit(self, rgb_array):
        """Submit the newest frame without ever blocking the caller."""

        now = time.monotonic()
        with self._condition:
            if self._stopping or now < self._next_submission_time:
                return False
            # Keep only one pending frame. A newer frame replaces an older one
            # if inference cannot keep up with the camera.
            self._pending_frame = (rgb_array, self._generation)
            self._next_submission_time = now + self.minimum_interval
            self._condition.notify()
        return True

    def get_latest(self):
        """Return the latest completed detections and surface worker errors."""
        ...
        with self._condition:
            if self._error is not None:
                ...
                self._error = None
                raise RuntimeError(
                    'NavisVision background inference failed: %s' % error
                ) from error
            return list(self._latest_detections)

    def reset(self):
        """Discard queued/results data and reset temporal tracking safely."""
    ...
    def _run(self):
        while True:
            ...
            if reset_requested:
                self.runtime.reset_temporal_filter()
            if pending_frame is None:
                continue
            rgb_array, generation = pending_frame

            try:
                detections = self.runtime.predict(rgb_array)
                if self.classifier_runtime is not None:
                    detections = self.classifier_runtime.classify_detections(
                        rgb_array,
                        detections,
                    )
                with self._condition:
...
def find_local_carla_server():
    ...
    while True:
        candidates = (
            os.path.join(directory, 'CarlaUE4.exe'),
            os.path.join(directory, 'CARLA_0.9.16', 'CarlaUE4.exe'),
        )
...
def connect_or_start_server(args):
    ...
    if args.host not in ('127.0.0.1', 'localhost', '::1'):
        raise RuntimeError('CARLA server is not reachable at %s:%s' % (args.host, args.port))
    ...
    command = [server_path, '-carla-rpc-port=%s' % args.port]
    if not args.show_server_window:
...
def game_loop(args):
    ...
    vision_runtime = None
    vision_classifier_runtime = None
    vision_worker = None
    vision_camera = None
    vision_detections = []
    ...
    try:
        ...
        if args.navisvision_model:
            ...
            detector_name = vision_runtime.metadata.get(
                'model_name',
                'NavisVision',
            )
            hud.notification(
                '%s loaded (%s); drawing detected objects' %
                (detector_name, vision_runtime.device),
                seconds=6.0,
            )
            print(
                '%s loaded on %s. Candidate confidence: %.2f; '
        ...
        if args.navisvision_classifier:
            try:
                from navisvision_model import TrafficControlClassifierRuntime
                vision_classifier_runtime = TrafficControlClassifierRuntime(
                    args.navisvision_classifier,
                    device=(
                        vision_runtime.device
                        if vision_runtime is not None else None
                    ),
                )
            except Exception as error:
                raise RuntimeError(
                    'Could not load the traffic-control classifier '
                    'checkpoint: %s' % error
                ) from error

            classifier_name = vision_classifier_runtime.metadata.get(
                'model_name',
                'Traffic-control classifier',
            )
            hud.notification(
                '%s loaded; comparing classifications per detection' %
                classifier_name,
                seconds=6.0,
            )
            print(
                '%s loaded on %s. Each eligible NavisVision box will show '
                'DET and CLS confidence.' %
                (classifier_name, vision_classifier_runtime.device)
            )

        if vision_runtime is not None:
            vision_worker = NavisVisionInferenceWorker(
                vision_runtime,
                classifier_runtime=vision_classifier_runtime,
                maximum_fps=args.navisvision_fps,
            )
            print(
                'NavisVision inference runs asynchronously at up to %.1f FPS; '
                'CARLA rendering and simulation do not wait for detections.' %
                args.navisvision_fps
            )
        ...
        while True:
            ...
            if (simulation_advanced and
                ...
                if (rgb_array is not None and
                    ...
                    vision_worker.submit(rgb_array)
                    vision_last_rgb_array = rgb_array
                vision_detections = vision_worker.get_latest()
        ...
        if server_process:
            print('Stopping the automatically started CARLA server...')
            stop_local_carla_server(server_process)
...
def main():
    ...
    argparser.add_argument(
        ...
        default=None,
        help=(
            'draw detections from a NavisVision or Faster R-CNN checkpoint'))
    argparser.add_argument(
        '--navisvision-classifier',
        metavar='CHECKPOINT',
        default=None,
        help=(
            'optionally classify every NavisVision traffic-control crop with '
            'a transfer-learned ResNet-18 or ViT-B/16 checkpoint'))
    argparser.add_argument(
    ...
    argparser.add_argument(
        ...
        type=float,
        help=(
            'maximum background detector inference rate; CARLA remains '
            'uncapped (default: 5.0)'))
    argparser.add_argument(
    ...
    if not 0.5 <= args.navisvision_fps <= 60.0:
        argparser.error('--navisvision-fps must be between 0.5 and 60.0')
    ...
    if args.navisvision_classifier and not args.navisvision_model:
        argparser.error(
            '--navisvision-classifier requires --navisvision-model because '
            'the classifier needs detector bounding boxes')
